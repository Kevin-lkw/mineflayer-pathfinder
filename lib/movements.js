...
class Movements {
  ...
  getMoveDiagonal (node, dir, neighbors) {
    let cost = Math.SQRT2 + 0.3 // move cost
    const toBreak = []
    ...
    // add regulation to stop view influence by Block C1/C2
    // stop the bot to move when the block C1/C2 is physical
    //    _|   <-do not move the turn like this
    //  _|
    if (blockC1.physical || blockC2.physical) {
      return
    }
    //finish modification
    if (cost1 < cost2) {
